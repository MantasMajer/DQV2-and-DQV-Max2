...
class DQV2(DQV):
    ...
    def __init__(self, state_dim, action_dim, hidden_dim=64, lr=0.05, q_weight=0.5, conv=False):
        super().__init__(state_dim, action_dim, hidden_dim, lr, conv=conv)
        self.q_weight = q_weight

    def update_V(self, state, y):
        y_pred = torch.mul(self.V_net.model(state), (1 - self.q_weight))
        loss = self.V_net.criterion(y_pred, Variable(torch.Tensor(y)))
    ...
    def replay_fast(self, memory, size, gamma=0.9):
        ...
        if len(memory) >= size:
            ...
            v_values_next[:, 0] = v_values_next[:, 0] - torch.mul(all_q_values[range(len(all_q_values)), actions], self.q_weight)
